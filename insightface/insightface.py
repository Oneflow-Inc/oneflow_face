...
parser.add_argument(
    "-wd", "--weight_decay", type=float, default=0.0005, required=False
)
parser.add_argument("-m", "--margin", type=float, default=0.35, required=False)
parser.add_argument(
    "-ms", "--margin_s", type=float, default=64, required=False
)
parser.add_argument(
    "-easy", "--easy_margin", type=int, default=0, required=False
)
parser.add_argument(
    "-network", "--network", type=str, default="resnet100", required=False
)
parser.add_argument(
    "-loss_type", "--loss_type", type=str, default="softmax", required=False
)
parser.add_argument(
    "-mp", "--model_parallel", type=int, default=0, required=False
)
parser.add_argument(
    "-l", "--loss_print_steps", type=int, default=1, required=False
)
parser.add_argument(
    "-s", "--part_name_suffix_length", type=int, default=-1, required=False
)
parser.add_argument("-tbn", "--total_batch_num", type=int, required=True)
parser.add_argument(
    "-snapshot_num", "--num_of_batches_in_snapshot", type=int, required=True
)
parser.add_argument("-models", "--models_name", type=str, required=False)
parser.add_argument(
    "-m1", "--loss_m1", type=float, default=1.0, required=False
)
parser.add_argument(
    "-m2", "--loss_m2", type=float, default=0.5, required=False
)
parser.add_argument(
    "-m3", "--loss_m3", type=float, default=0.0, required=False
)
