...
for param_name in arg_params.keys():
  output_path = os.path.join(of_model_path, '_'.join(param_name.split('_')[0:-1]))
  ...
  if param_name.split('_')[-2] == 'conv2d':
    ...
    output_path = os.path.join(of_model_path, '_'.join(param_name.split('_')[0:-2]))
    os.mkdir(output_path + '-' + param_name.split('_')[-1])
    if param_name.split('_')[-1] == 'weight':
      _SaveWeightBlob2File(weight,output_path + '-weight', "out")
    elif param_name.split('_')[-1] == 'bias':
...
for param_name in aux_params.keys():
  ...
  if param_name.split('_')[-3] == 'batchnorm' or param_name.split('_')[-3] == 'fc1':
    if param_name.split('_')[-1] == 'mean':
      ...
      _SaveWeightBlob2File(weight,output_path + '-moving_variance', "out")
    else:
      print(param_name,"error error")
  else:
    print(param_name,"error error")

global_step = os.path.join(of_model_path, 'System-Train-TrainStep-insightface_train_job')
