...
parser.add_argument("-mx_load_prefix", "--mxnet_load_prefix", type=str, required=False)
parser.add_argument("-mx_load_epoch", "--mxnet_load_epoch", type=int, required=False)
parser.add_argument("-of", "--of_model_dir", type=str, required=False)
parser.add_argument("-mx_save_prefix", "--mxnet_save_prefix", type=str, required=False)
parser.add_argument("-mx_save_epoch", "--mxnet_save_epoch", type=int, required=False)
...
for param_name in arg_params.keys():
  ...
  if 'conv' in param_name.split('_')[-2]:
    if param_name.split('_')[-1] == 'weight':
      of_weight = np.fromfile(of_model_path + "-weight/out", dtype=np.float32)
    elif param_name.split('_')[-1] == 'bias':
      of_weight = np.fromfile(of_model_path+"-bias/out", dtype=np.float32)
    else:
  ...
  elif 'bn' in param_name.split('_')[-2] or param_name.split('_')[-2] == 'sc' or param_name.split('_')[-2] == 'batchnorm':
    if param_name.split('_')[-1] == 'gamma':
      of_weight = np.fromfile(of_model_path+"-gamma/out", dtype=np.float32)
    elif param_name.split('_')[-1] == 'beta':
      of_weight = np.fromfile(of_model_path+"-beta/out", dtype=np.float32)
    else:
      print(param_name,"error error")
    of_weight=of_weight.reshape(arg_params[param_name].asnumpy().shape)  
    arg_params[param_name] = mx.nd.array(of_weight)
    ...
    if param_name.split('_')[-1] == 'gamma':
      of_weight = np.fromfile(of_model_path+"-alpha/out", dtype=np.float32)
    else:
      print(param_name,"error error")
    of_weight=of_weight.reshape(arg_params[param_name].asnumpy().shape)  
    arg_params[param_name] = mx.nd.array(of_weight)
    ...
    of_weight=of_weight.reshape(arg_params[param_name].asnumpy().shape)  
    arg_params[param_name] = mx.nd.array(of_weight)
  elif param_name.split('_')[-2] == 'fc1' or param_name.split('_')[-2] == 'fc7':
    if param_name.split('_')[-1] == 'weight':
      of_weight = np.fromfile(of_model_path+"-weight/out", dtype=np.float32)
      of_weight=of_weight.reshape(arg_params[param_name].asnumpy().shape)  
      arg_params[param_name] = mx.nd.array(of_weight)
...
for param_name in aux_params.keys():
  of_model_path = of_dir +'_'.join(param_name.split('_')[0:-2])
  if 'bn' in param_name.split('_')[-3] or 'fc1' in param_name.split('_')[-3] or param_name.split('_')[-3] == 'sc' or param_name.split('_')[-3] == 'batchnorm':
    if param_name.split('_')[-1] == 'mean':
      of_weight = np.fromfile(of_model_path+"-moving_mean/out", dtype=np.float32)
      of_weight=of_weight.reshape(aux_params[param_name].asnumpy().shape)  
      aux_params[param_name] = mx.nd.array(of_weight)
    elif param_name.split('_')[-1] == 'var':
      of_weight = np.fromfile(of_model_path+"-moving_variance/out", dtype=np.float32)
      of_weight=of_weight.reshape(aux_params[param_name].asnumpy().shape)  
      aux_params[param_name] = mx.nd.array(of_weight)
    else:
      print(param_name,"error error")
