...
class VisionTransformer(nn.Module):
    ...
    def random_masking(self, x, mask_ratio=0.1):
        ...
        noise = flow.rand((N, L), placement=x.placement, sbp=x.sbp)  # noise in [0, 1]

        # sort noise for each sample
        # ascend: small is keep, large is remove
        ids_shuffle = flow.argsort(noise, dim=1)
        ids_restore = flow.argsort(ids_shuffle, dim=1)

        # keep the first subset
        ids_keep = ids_shuffle[:, :len_keep]
        x_masked = flow.gather(
            x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))

        # generate the binary mask: 0 is keep, 1 is remove
        # mask = flow.ones([N, L], device=x.device)
        mask = flow.ones([N, L], placement=x.placement, sbp=x.sbp)
        mask[:, :len_keep] = 0
    ...
    def forward_features(self, x):
        ...
        if self.training and self.mask_ratio > 0:
            ...
            x_ = flow.cat([x[:, :, :], mask_tokens], dim=1)  # no cls token
            x_ = flow.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x.shape[2]))  # unshuffle
            x = x_
        return flow.reshape(x, (B, self.num_patches * self.embed_dim))
...
#         img_size=112, patch_size=9, num_classes=0, embed_dim=256, depth=12,
...
if __name__ == "__main__":
    ...
    class Graph(flow.nn.Graph):
        ...
        def build(self, x):
            return self.model(x)
    graph = Graph()
    y = graph(flow.randn(3, 3, 112, 112))
    print(y.shape)
    # print(model(flow.randn(3,3,112,112)).shape)