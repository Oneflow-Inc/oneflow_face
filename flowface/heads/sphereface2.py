...
class SphereFace2(nn.Module):
    ...
    def __init__(
        ...
        self.w = flow.nn.Parameter(
            flow.empty(num_classes, embedding_size, sbp=sbp_w, placement=placement)
        )
        ...
        if is_global:
            placement = flow.env.all_device_placement("cuda")
            self.w = nn.Parameter(
                flow.empty(num_classes, embedding_size, sbp=flow.sbp.split(0), placement=placement)
            )
        ...
        # init bias
        z = alpha / ((1.0 - alpha) * (num_classes - 1.0))
        if magn_type == "C":
    ...
    def forward(self, x, y):
        ...
        one_hot = flow.scatter(one_hot, 1, y.view(-1, 1), 1.0)
        ...
        with flow.no_grad():
            if self.magn_type == "C":
                ...
                theta_m = flow.acos(cos_theta.clamp(-1 + 1e-5, 1.0 - 1e-5))
                ...
                m_theta = flow.acos(cos_theta.clamp(-1 + 1e-5, 1.0 - 1e-5))
        ...
        loss = flow._C.binary_cross_entropy_with_logits_loss(logits, one_hot, weight, None, "mean")
