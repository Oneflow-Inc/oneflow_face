...
from flowface.utils.utils_distributed_sampler import DistributedSampler
from flowface.utils.utils_distributed_sampler import get_dist_info, worker_init_fn
...
class MXFaceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(label, numbers.Number):
            label = label[0]
        label = flow.tensor(label, dtype=torch.long)
...
if __name__ == "__main__":
    path = "/workspace/data/faces_emore"
    dataset = get_dataloader(path, 0, 128)